...
public class connectActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    private OneSheeldTestingCallback testingCallback = new OneSheeldTestingCallback() {
        ...
        public void onFirmwareTestResult(final OneSheeldDevice device, final boolean isPassed) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Firmware test result: " + (isPassed ? "Correct" : "Failed"), Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        public void onLibraryTestResult(final OneSheeldDevice device, final boolean isPassed) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Library test result: " + (isPassed ? "Correct" : "Failed"), Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        public void onFirmwareTestTimeOut(final OneSheeldDevice device) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Error, firmware test timeout!", Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        public void onLibraryTestTimeOut(final OneSheeldDevice device) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Error, library test timeout!", Toast.LENGTH_SHORT).show();
                }
            });
        }
    };
    private OneSheeldRenamingCallback renamingCallback = new OneSheeldRenamingCallback() {
        ...
        public void onRenamingAttemptTimeOut(final OneSheeldDevice device) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Error, renaming attempt failed, retrying!", Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        public void onAllRenamingAttemptsTimeOut(final OneSheeldDevice device) {
            ...
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Error, all renaming attempts failed!", Toast.LENGTH_SHORT).show();
                }
            });
        ...
        public void onRenamingRequestReceivedSuccessfully(final OneSheeldDevice device) {
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, device.getName() + ": Renaming request received successfully!", Toast.LENGTH_SHORT).show();
        ...
        public void onBaudRateQueryResponse(final OneSheeldDevice device, final SupportedBaudRate supportedBaudRate) {
            if (isBaudRateQueried) {
                uiThreadHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(connectActivity.this, device.getName() + (supportedBaudRate != null ? ": Current baud rate: " + supportedBaudRate.getBaudRate() : ": Device responded with an unsupported baud rate"), Toast.LENGTH_SHORT).show();
                    }
                });
    ...
    private OneSheeldErrorCallback errorCallback = new OneSheeldErrorCallback() {
        ...
        public void onError(final OneSheeldDevice device, final OneSheeldError error) {
            ...
            uiThreadHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(connectActivity.this, "Error: " + error.toString() + (device != null ? " in " + device.getName() : ""), Toast.LENGTH_SHORT).show();
                }
            });
        }
    };
    ...
    private OneSheeldFirmwareUpdateCallback firmwareUpdateCallback = new OneSheeldFirmwareUpdateCallback() {
        ...
        public void onSuccess(final OneSheeldDevice device) {
            uiThreadHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(connectActivity.this, device.getName() + ": Firmware update succeeded!", Toast.LENGTH_SHORT).show();
                }
            });
        }
        ...
        public void onFailure(final OneSheeldDevice device, final boolean isTimeOut) {
            uiThreadHandler.post(new Runnable() {
                ...
                public void run() {
                    firmwareUpdateProgressDialog.dismiss();
                    Toast.makeText(connectActivity.this, device.getName() + ": Firmware update failed!" + (isTimeOut ? "Time-out occurred!" : ""), Toast.LENGTH_SHORT).show();
                }
            });
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sidebar_connect);
                ...
                locationPremissionExplanationDialog.setMessage("Bluetooth scan needs location permission.").setPositiveButton("Allow", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ActivityCompat.requestPermissions(connectActivity.this,
                                ...
                                connectActivity.PERMISSION_REQUEST_CODE);
                    }
                }).setNegativeButton("Deny", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        Toast.makeText(connectActivity.this, "We can't start scanning until you grant the Location permission.", Toast.LENGTH_SHORT).show();
                    }
                }).create();
            ...
            if (permissions.length > 0) {
                if (permissions[0].equals(Manifest.permission.ACCESS_COARSE_LOCATION)) {
                    if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        Toast.makeText(connectActivity.this, "You can start scanning now.", Toast.LENGTH_SHORT).show();
                    } else {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            if (shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_COARSE_LOCATION))
                                Toast.makeText(connectActivity.this, "We can't start scanning until you grant the Location permission.", Toast.LENGTH_SHORT).show();
                            else
                                Toast.makeText(connectActivity.this, "We can't start scanning until you grant the Location permission from the settings.", Toast.LENGTH_SHORT).show();
